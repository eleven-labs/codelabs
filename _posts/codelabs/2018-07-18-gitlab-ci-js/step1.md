...
start: ## Run container: default dev
	$(info --> Run containers: ${NODE_ENV})7
	@docker run \
		--rm \
		--detach \
		--user node \
...
run: ## Run command inside this projet if not running
...
e2e: ## Execute test inside container
	...
	@docker run \
		...
		--volume "${CURDIR}:${APP_DIR}" \
		cypress/base:8 \
		sh -c "yarn install && yarn run cypress install && yarn run test:e2e --headless"
